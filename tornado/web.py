...
class RequestHandler(object):
    ...
    def render_string(self, template_name, **kwargs):
        ...
        """
        return "".join(self._render_iter(template_name, **kwargs))

    def render_async(self, template_name, **kwargs):
        """Renders the template with the given arguments as async response."""
        chunks = self._render_iter(template_name, **kwargs)
        for chunk in output:
            self.write(chunk)
            self.flush()

    def _render_iter(self, template_name, **kwargs):
